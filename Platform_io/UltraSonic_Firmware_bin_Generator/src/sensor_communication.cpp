            ...
            if (Wire.endTransmission(false) == 0) {
                ...
                if (Wire.available() >= EEPROM_SIZE) {
                    ...
                    if (eepromData == "OSI") {
                        ...
                        sensorType = "ULTRASONIC"; // Changed from TOF to ULTRASONIC
                    } else {
                        sensorType = "ULTRASONIC"; // Default to ultrasonic
                    }
...
uint16_t readUltrasonicDistanceCM() {
    ...
    if (distance_mm >= calibration.minValidReading * 10 && 
        ...
        if (calibrated < 20) calibrated = 20; // 2cm min
        if (calibrated > MAX_DISTANCE_MM) calibrated = MAX_DISTANCE_MM;
