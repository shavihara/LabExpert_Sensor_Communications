...
void handleStatus(AsyncWebServerRequest *request) {
    ...
    doc["connected"] = true;
    doc["sensor_type"] = sensorType + "_GPIO";
...
void handleConfigure(AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("plain", true)) {
        ...
        if (doc.containsKey("calibration")) {
            calibration.offsetCM = doc["calibration"]["offset"] | 0.0;
...
void handleCalibrate(AsyncWebServerRequest *request) {
    if (request->hasParam("plain", true)) {
        ...
        if (deserializeJson(doc, body) == DeserializationError::Ok) {
            calibration.offsetCM = doc["offset"] | 0.0;
            calibration.scaleFactor = doc["scale"] | 1.0;
            
            Serial.printf("Calibration updated: offset=%.2fcm, scale=%.4f\n", 
                calibration.offsetCM, calibration.scaleFactor);
