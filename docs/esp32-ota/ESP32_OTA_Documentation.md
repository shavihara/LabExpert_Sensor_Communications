# ESP32 OTA (ESP_32_OTA) Mechanism — Comprehensive Documentation
...
- Web upload (multipart form):
  - `POST /update` writes file chunks: `Update.begin(UPDATE_SIZE_UNKNOWN)` → `Update.write(upload.buf)` → `Update.end(true)`, then restart (`ESP_32_OTA/src/main.cpp:211-242`).
- Backend push API (JSON hex streaming):
  ...
  - `POST /ota/write { offset, size, data(hex) }` decodes hex → bytes and calls `Update.write(bytes)` with strict size checks (`ESP_32_OTA/src/main.cpp:288-321`).
...
- See `architecture.drawio` and `architecture.png` (to be generated). Components and connections:
  - ESP32 OTA Bootloader (`ota_0`) ↔ Browser (HTTP form upload)
  - ESP32 OTA Bootloader ↔ Backend (HTTP push API)
  - ESP32 OTA Bootloader ↔ UDP discovery (ports 8888/8889)
  - Application firmware (`ota_1`) ↔ MQTT broker (status/control)
